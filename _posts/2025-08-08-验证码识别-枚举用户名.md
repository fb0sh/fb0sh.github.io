...
with open("./phone.txt", "r") as f:
    with open("./log.txt", "w", buffering=1, encoding="UTF-8") as fw:
        for phone in f.readlines():
            ...
            url = f"https://szsj.cmdi.10086.cn/iauth/code?randomStr={randomStr}"
            ...
            url = f"https://szsj.cmdi.10086.cn/iauth/smsCode?username={phone}&code={code}&randomStr={randomStr}"
            res = requests.get(url)
            fs = f"{phone}\t{res.text}\n\n"
            fw.write(fs)
            print(fs)
